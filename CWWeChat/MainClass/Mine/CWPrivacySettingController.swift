...
class CWPrivacySettingController: CWBaseTableViewController {
    ...
    func setupItemData() {
        ...
        let item3 = BoolItem(title: "向我推荐通讯录好友", value: true) { (_) in
        ...
        let item8 = BoolItem(title: "允许陌生人查看十条朋友圈", value: true) { (_) in
            
        }
        let section4 = Section(headerTitle:"朋友圈", items: [item5, item6, item7, item8])
        tableViewManager.append(section4)

        let item9 = BoolItem(title: "开启朋友圈入口", value: true) { (_) in
        ...
        let item10 = BoolItem(title: "朋友圈更新提醒", value: true) { (_) in
